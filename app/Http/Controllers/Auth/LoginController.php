...
class LoginController extends Controller
{
    ...
    return response()->json([
        ...
        'user' => [
            ...
            'is_technician' => $userfind->is_technician,
            'roles' => $userfind->getRoleNames()
                        ...
                        $isTechnicianLogin = $request->has('is_technician') && $request->input('is_technician') == '1';

                        if ($userfind->hasRole('admin')) {
                            Log::info('Redirecting to admin dashboard', ['user_id' => $userfind->id]);
                            return redirect()->route('admin.dashboard')->with('success', "Login successfully!");
                        } else if ($userfind->hasRole('artist') || $userfind->is_technician || $isTechnicianLogin) {
                            Log::info('Redirecting to artist portal', ['user_id' => $userfind->id, 'is_technician_login' => $isTechnicianLogin]);

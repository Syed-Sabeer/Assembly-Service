...
<div class="page-body">
          ...
          <div class="container-fluid list-product-view product-wrapper">
            ...
            <div class="filter-section">
                <form method="GET" action="{{ route('admin.product.index') }}" class="d-inline-flex gap-2 align-items-center">
                    <input type="text" name="search" class="form-control" placeholder="Search..." value="{{ $search }}" style="width: 200px;">
                    <button type="submit" class="btn btn-primary"><i class="fa fa-search"></i></button>
                    <a href="{{ route('admin.product.index') }}" class="btn btn-secondary"><i class="fa fa-redo"></i></a>
                    <a class="btn btn-success f-w-500" href="{{route('admin.product.add')}}"><i class="fa fa-plus pe-2"></i>Add Product</a>
                </form>
            </div>
                        ...
                        <table class="table" >
                          ...
                          <tbody>
                            ...
                            <tr class="product-removes">
                              ...
                              <td>
                                @if($product->f_image)
                                  <img src="{{ asset($product->f_image) }}" alt="{{ $product->title }}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 5px;">
                                @else
                                ...
                                <td>
                                <p class="c-o-light">{{$product->title}}</p>
                              </td>
  ...
  <a class="square-white" href="{{ route('admin.product.edit', $product->id) }}">
    <svg>
  ...
  <form action="{{ route('admin.product.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this product?');" style="display:inline;">
    @csrf
    @method('DELETE')

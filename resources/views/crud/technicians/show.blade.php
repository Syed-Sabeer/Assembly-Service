...
<style>
    ...
    .dark-only .profile-header {
        background: linear-gradient(135deg, #4c1d95 0%, #7c3aed 100%);
    }
    .dark-only .cert-item, .dark-only .edu-item, .dark-only .project-item {
        background: #0f172a !important;
        border-left-color: #818cf8 !important;
    }
    .dark-only .cert-item h5, .dark-only .edu-item h5, .dark-only .project-item h5 {
        color: #ffffff !important;
    }
    .dark-only .cert-item p, .dark-only .edu-item p, .dark-only .project-item p {
...
<div class="page-body">
    ...
    <div class="container-fluid">
        ...
        <!-- Resume -->
        <div class="info-card">
            <h4><i class="fa fa-file-pdf"></i> Legal Documents / Resume</h4>
            @if($technician->resume)
                @php
                    $resumePath = Storage::disk('public')->exists($technician->resume) ? asset('storage/' . $technician->resume) : '';
                    $resumeFileName = basename($technician->resume);
                @endphp
                @if($resumePath)
                    <div class="info-item">
                        <span class="info-label">Resume File:</span>
                        <span class="info-value">
                            <a href="{{ $resumePath }}" target="_blank" class="btn btn-primary btn-sm">
                                <i class="fa fa-download"></i> Download Resume
                            </a>
                            <span class="text-muted ms-2">({{ $resumeFileName }})</span>
                        </span>
                    </div>
                @else
            ...
            @else
                <p style="color: #dc3545; font-weight: 600;">
                    <i class="fa fa-exclamation-triangle"></i> No resume uploaded
                </p>
            @endif
        </div>
    </div>

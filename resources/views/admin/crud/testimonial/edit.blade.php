...
<form method="POST" action="{{ $isEdit ? route('admin.testimonial.update', $testimonial->id) : route('admin.testimonial.store') }}">
  @csrf
  @if ($isEdit)
    @method('PUT')
  @endif
  ...
  <div class="col-md-12">
    <label class="form-label" for="validationProductTitle">Type</label>
    <input
      class="form-control"
      name="type"
      id="validationProductTitle"
    ...
    <textarea 
        ...
        id="validationProductTitle" 
        rows="4" 
        placeholder="Write your review...">{{ old('review', $testimonial->review ?? '') }}</textarea>
</div>

...
<form method="POST" action="{{ $isEdit ? route('admin.service.update', $service->id) : route('admin.service.store') }}" enctype="multipart/form-data">
  ...
  <div class="col-md-6">
    <label class="form-label" for="service-bg-image">Background Image</label>
    <input class="form-control" name="bg_image" id="service-bg-image" type="file" accept="image/*">
    @if($isEdit && $service->bg_image)
      <div class="mt-2">
        <small class="text-muted">Current Background Image:</small><br>
        <img src="{{ asset($service->bg_image) }}" alt="Current Background Image" style="width: 100px; height: 100px; object-fit: cover;" class="mt-1">
      </div>
    @endif
  </div>
